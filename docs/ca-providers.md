...
Encrypt staging when the request was a staging one) and logs a warning. The
**CA** column on the Certificates page names the authority each certificate was
issued with, so a fallback is visible without going through a log; the same
value is `ca_provider` in the API response and in the certificate's metadata.
Certificates issued before CertMate recorded the CA show `—` there rather than
a guess.
