...
- **Configurable certificate key type/size**: every cert was hardcoded to RSA-2048 because `CAManager.build_certbot_command` never passed `--key-type` or `--rsa-key-size` to certbot. Operators with compliance requirements (RSA-3072/4096) or modern stacks (ECDSA, smaller certs and faster handshakes) had to patch the code. The settings page now exposes a global default (`default_key_type` / `default_key_size` / `default_elliptic_curve`) and the certificate creation form has an optional per-cert override under "Advanced Options". Default for upgraded installs stays at `rsa`/`2048` so behaviour does not change unless the operator opts in. RSA accepts 2048/3072/4096; ECDSA accepts `secp256r1` and `secp384r1`. Validation runs on every save and every cert creation so a contradictory shape (e.g. `key_type=rsa` with an `elliptic_curve` override) is rejected with a 400 instead of failing later inside certbot.

  Renewals preserve the original key shape across the two deployment topologies CertMate supports:

  1. **PVC-backed deployments** (the default `certbot renew` fast path): certbot persists `--key-type`/`--rsa-key-size`/`--elliptic-curve` into `certificates/{domain}/renewal/<domain>.conf` at create time and reuses them on `certbot renew --cert-name`. No CertMate-side action needed.
  2. **Ephemeral-filesystem deployments with a remote storage backend** (Docker without a PVC on `certificates/`, K8s pods on `emptyDir`, anywhere the local FS is recycled while Azure Key Vault / AWS Secrets Manager / Vault / Infisical hold the source of truth): two layers cooperate. Startup runs `CertificateManager.hydrate_from_storage`, which walks `settings.domains` and writes the PEMs plus an extended `metadata.json` (key shape, DNS plugin, account, CA, SAN list) back to local disk for any domain that's missing locally. The certbot `renewal/<domain>.conf` is *not* restored — the storage backend never carried it — so on the first renew after restart `renew_certificate` detects the missing conf and rebuilds the cert from `metadata.json` via `create_certificate(force=True, …)`. The original key shape, DNS plugin and SAN list are honoured.
