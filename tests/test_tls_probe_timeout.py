...
They stub `_resolve_and_guard` as well as `socket.create_connection`. The
probe resolves the name and checks the address before connecting, so patching
only the connect would leave a real DNS lookup inside a unit test — which is
both a hidden network dependency and, for a name that does not resolve, a
failure that has nothing to do with what is being measured here.
"""
...
def test_slow_probe_logs_warning(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="modules.api.resources"):
        with patch('modules.api.tls_probe._resolve_and_guard',
                   return_value=(2, '127.0.0.1', None)), \
             patch('modules.api.tls_probe.socket.create_connection',
